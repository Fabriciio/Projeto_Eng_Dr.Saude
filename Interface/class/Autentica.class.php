...
class Autentica extends Conexao{
	...
	public function __get($name){
		...
		public function Validar_Usuario(){
			...
			$resultado = $pdo->select("SELECT * FROM funcionarios WHERE email_f = '".$this->email."' AND senha_f = '".$this->pass."'");
			//desconectamos
			$pdo->desconectar();
			...
			if(count($resultado)){
				foreach ($resultado as $res) {
					...
					$_SESSION['id_users'] = $res['cpf_f'];
					$_SESSION['nome'] = $res['login_f'];
					$_SESSION['email'] = $res['email_f'];
					$_SESSION['pass'] = $res['senha_f'];
					$_SESSION['tip'] = $res['tipo_f'];
					$_SESSION['logado'] = 'S';
			}
